import os

# AES S-box (Substitution box) for byte substitution during encryption
s_box = [
    
]

# Inverse S-box (AES standard) for byte substitution during decryption
inv_s_box = [
    
]

# Rcon (Round Constant) for in key expansion
r_con = [
    
]

# substitutes each byte in the state with the corresponding byte in the S-box
def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]
    return state

def inv_sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = inv_s_box[state[i][j]]
    return state

# shiftRows transformation: shifts the rows of the state matrix
def shift_rows(state):
    state[1] = state[1][1:] + state[1][:1] #Shift second row by 1 byte
    state[2] = state[2][2:] + state[2][:2] #Shift third row by 2 byte
    state[3] = state[3][3:] + state[3][:3] #Shift fourth row by 3 byte
    return state

def inv_shift_rows(state):
    state[1] = state[1][-1:] + state[1][:-1]
    state[2] = state[2][-2:] + state[2][:-2]
    state[3] = state[3][-3:] + state[3][:-3]
    return state

# mixes the columns of the state matrix using matrix multiplicatio
def mix_columns(state):
    for i in range(4):
        s0 = state[0][i]
        s1 = state[1][i]
        s2 = state[2][i]
        s3 = state[3][i]
        state[0][i] = multiply(0x02, s0) ^ multiply(0x03, s1) ^ s2 ^ s3
        state[1][i] = s0 ^ multiply(0x02, s1) ^ multiply(0x03, s2) ^ s3
        state[2][i] = s0 ^ s1 ^ multiply(0x02, s2) ^ multiply(0x03, s3)
        state[3][i] = multiply(0x03, s0) ^ s1 ^ s2 ^ multiply(0x02, s3)
    return state

def inv_mix_columns(state):
    for i in range(4):
        s0 = state[0][i]
        s1 = state[1][i]
        s2 = state[2][i]
        s3 = state[3][i]
        state[0][i] = multiply(0x0e, s0) ^ multiply(0x0b, s1) ^ multiply(0x0d, s2) ^ multiply(0x09, s3)
        state[1][i] = multiply(0x09, s0) ^ multiply(0x0e, s1) ^ multiply(0x0b, s2) ^ multiply(0x0d, s3)
        state[2][i] = multiply(0x0d, s0) ^ multiply(0x09, s1) ^ multiply(0x0e, s2) ^ multiply(0x0b, s3)
        state[3][i] = multiply(0x0b, s0) ^ multiply(0x0d, s1) ^ multiply(0x09, s2) ^ multiply(0x0e, s3)
    return state

def multiply(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80 # Check if the most significant bit (MSB) of 'a' is set (1)
        a <<= 1 # Left shift 'a'
        if hi_bit_set:
            a ^= 0x1b
        b >>= 1 # Right shift 'b'
    return p & 0xff # Ensure the result is an 8-bit value

# add the round key to the state using XOR operation
def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]
    return state

# expands the key into a key schedule for all rounds
def key_expansion(key, key_size):
    nk = key_size // 32 #Calculate the number of 32-bit words
    nr = nk + 6 #Calculate the number of rounds (nr)
    w = [[0] * 4 for _ in range(4 * (nr + 1))] #initialize empty array base on how many require words
    for i in range(nk): #Copy the original key into the first nk words of the key schedule
        w[i] = [key[4*i], key[4*i+1], key[4*i+2], key[4*i+3]]
    for i in range(nk, 4 * (nr + 1)):
        temp = w[i-1]
        if i % nk == 0: #g func
            temp = [temp[1], temp[2], temp[3], temp[0]] # Rotate left
            temp = [s_box[b] for b in temp] # SubWord
            temp[0] ^= r_con[i//nk] # XOR with round constant
        elif nk > 6 and i % nk == 4: # For AES-192 and AES-256, additional S-Box transformation
            temp = [s_box[b] for b in temp]
        w[i] = [w[i-nk][j] ^ temp[j] for j in range(4)] # Generate the new word by XOR-ing temp with the word `nk` positions earlier
    return w

# converts a byte array into a 4x4 state matrix
def bytes_to_state(byte_array):
    return [[byte_array[i + 4*j] for j in range(4)] for i in range(4)]

# converts a 4x4 state matrix into a byte array
def state_to_bytes(state):
    return [state[i][j] for j in range(4) for i in range(4)]

def encrypt_block(plaintext, key_schedule, nr):
    state = bytes_to_state(plaintext)
    state = add_round_key(state, key_schedule[:4])
    for round in range(1, nr):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, key_schedule[4*round:4*(round+1)])
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, key_schedule[4*nr:4*(nr+1)])
    return state_to_bytes(state)

def decrypt_block(ciphertext, key_schedule, nr):
    state = bytes_to_state(ciphertext)
    state = add_round_key(state, key_schedule[4*nr:4*(nr+1)])
    for round in range(nr-1, 0, -1):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, key_schedule[4*round:4*(round+1)])
        state = inv_mix_columns(state)
    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, key_schedule[:4])
    return state_to_bytes(state)

# pads the plaintext to be a multiple of 16 bytes
def pad(data):
    padding_length = 16 - (len(data) % 16)
    return data + bytes([padding_length] * padding_length)

# removes padding from the decrypted plaintext
def unpad(data):
    padding_length = data[-1]
    return data[:-padding_length]

def encrypt(plaintext, key, key_size):
    key_schedule = key_expansion(key, key_size) 
    nr = key_size // 32 + 6
    padded_plaintext = pad(plaintext)
    ciphertext = b''
    for i in range(0, len(padded_plaintext), 16): #range(start, stop, step)
        block = padded_plaintext[i:i+16]
        ciphertext += bytes(encrypt_block(block, key_schedule, nr))
    return ciphertext

def decrypt(ciphertext, key, key_size):
    key_schedule = key_expansion(key, key_size)
    nr = key_size // 32 + 6
    plaintext = b''
    for i in range(0, len(ciphertext), 16):
        block = ciphertext[i:i+16]
        plaintext += bytes(decrypt_block(block, key_schedule, nr))
    return unpad(plaintext)

def generate_symmetric_key(key_size):
    if key_size not in [128, 192, 256]:
        raise ValueError("Invalid key size. Choose 128, 192, or 256 bits.")

    key_bytes = key_size // 8 
    return os.urandom(key_bytes)